...
> 实现状态（2026-08-21 阶段 3 启动）：
> - **稳定性**：`runtime.start` 纳入启动 try；启动失败可弹窗；扫描/删除主机改为 AsyncCall（不再 GUI 线程卡死）；AsyncCall 增加 `superseded`；日志同时写文件+控制台
> - **观感**：侧栏/告警页留白与卡片圆角微调，表头与品牌样式加强
> - **未做**：Web UI 内嵌评估仍待产品反馈后再定
