...
| 问题归档 | AI 对话生成报告草稿，确认后发布飞书文档并写入 Bitable 工单 |
...
| **问题报告** | 报告草稿编辑、发布飞书文档、Bitable 工单同步与结案 |
| **设置** | SSH、大模型、飞书、巡检间隔等配置 |

关闭窗口或最小化时默认进入 **系统托盘后台运行**（右下角小图标），单击托盘图标可重新打开控制台；右键托盘图标可选择「退出」彻底关闭。
...
- 「列出所有服务及其状态」
- 「分析一下最近的告警」
- 「根据这次排查生成问题报告」（AI 调用 `create_problem_report` 生成草稿，需到【问题报告】页确认后发布）
...
将 AI 排查结论结构化归档，并与飞书文档、多维表格（Bitable）工单联动：

```
巡检 / 告警 / AI 对话 → 生成报告草稿 → 人工确认 → 飞书文档 + Bitable 工单 + 群通知
```

| 环节 | 说明 |
|------|------|
| **生成草稿** | AI 对话说「生成问题报告」，或告警页手动创建；P0/P1 告警可自动起草（可选） |
| **编辑确认** | 【问题报告】→「报告编辑」：修改标题、描述、Markdown 正文与负责人 |
| **发布飞书** | 创建飞书文档、写入 Bitable 工单行、向运维群发送链接通知 |
| **更新文档** | 已发布案例可「更新飞书」，覆盖原文档并同步工单字段 |
| **同步工单** | 从 Bitable 拉取最新状态与负责人；工单已关闭则自动结案 |
| **关闭案例** | 本地结案、Bitable 标为「已关闭」，并联动解决关联告警 |

在 **设置 → 问题报告 / 飞书归档** 中配置：归档文件夹 Token、Bitable App Token / Table ID、通知群 ID 等。发布前需人工确认，**不会自动发布到飞书**（除非单独开启 `auto_publish`）。

完整流程与字段说明见 [问题报告与飞书工单工作流](docs/ops-report-workflow.md)。
...
- **问题报告归档**：飞书文档 + Bitable 工单 + 群通知，支持同步与结案闭环
